function forecastViewModel() {
    ...
    // Locations
    (typeof(locations) == 'object' && ('length' in locations)) || (locations = []);
    ...
    self.allNotifications = ko.observable({});
    self.notifications =  ko.computed({
        read: function(){
            if (self.selectedLocationId() && typeof((self.allNotifications())[self.selectedLocationId()])!='undefined'){
                return (self.allNotifications())[self.selectedLocationId()];
            }
            else {
                return null;
            }
        ...
        write: function(value){
            var values = self.allNotifications();
            if (value){
                values[self.selectedLocationId()] = value;
            }
            else{
                delete values[self.selectedLocationId()];
            }
            self.allNotifications(values);
        }
    });
    var date = new Date();
...
//            // edit mode: place form instead of original row 
    ...
    self.updateLocation = function(locationForm) { 
        //fetch values
        var curLocation = self.locationEntry();
    ...
    self.deleteNotification = function(notification, event) {
        ...
        $.ajax({
            ...
            success: function() {
                ...
                var rows = self.allNotifications()[self.selectedLocationId()];
                for (var i in rows) {
                    var row = rows[i];
                    if (row.id == notification.id) {
                        self.allNotifications()[self.selectedLocationId()] = rows.splice(i, 1);
                    }
                }
    ...
    self.editNotification = function(notification, event) {
        ...
        if (notification.id > 0) {
            ...
            //create mode
            var entry = jQuery.extend({}, self.emptyNotificationRow);
            entry.location_id = self.selectedLocationId()
    ...
    self.updateNotification = function() { 
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var locationId = data.hasOwnProperty('locationId') ? data.locationId : null;
                var needToDelete = locationId != self.selectedLocationId();

                var rows = self.allNotifications();

                (typeof(rows[locationId]) == 'object') || (rows[locationId] = []);

                if (needToDelete && typeof(rows[self.selectedLocationId()]) == 'object') {
                    for (var i in rows[self.selectedLocationId()]) {
                        var row = rows[self.selectedLocationId()][i];
                        if (row.id == data.id) {
                            // remove record with another location
                            rows[self.selectedLocationId()].splice(i, 1);
                        }
                    }
                }

                // if object is new or moved - insert it into array
                if (needToDelete || !notification.hasOwnProperty('id')) {
                    rows[locationId].push(data);
                }

                self.allNotifications(rows);
                /*

                var rows = self.notifications();
                ...
                if (notification.hasOwnProperty('id')) {
                    ...
                    !!!! записывать напрямую в allNotifications!
                }
                // update the list
...
*/
    ...
    // on select another location action
    self.allNotifications.subscribe(function(rows) {
        for (var location in rows ) {
            for (var r in rows[location] ) {
                var newRow = rows[location][r];
